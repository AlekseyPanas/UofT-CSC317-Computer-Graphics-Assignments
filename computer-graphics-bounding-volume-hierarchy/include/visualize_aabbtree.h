  ...
  const std::shared_ptr<AABBTree> & tree)
{
  ...
  /**-0.53361024313, -0.38735633781, -0.29796950393     
DIR 0.02654740448, 0.18222716506, 0.691963120224*/

  // Debug, Displays a custom ray to determine a collision visually
  // EV.conservativeResize(EV.rows() + 2, Eigen::NoChange);
  // Eigen::RowVector3d origin = Eigen::RowVector3d(-0.53361024313, -0.38735633781, -0.29796950393);
  // EV.row(EV.rows() - 2) = origin;
  // EV.row(EV.rows() - 1) = origin + (Eigen::RowVector3d(0.02654740448, 0.18222716506, 0.691963120224) * 100);
  // EE.conservativeResize(EV.rows() + 1, Eigen::NoChange);
  // EE.row(EE.rows() - 1) << EV.rows() - 1, EV.rows() - 2;
