...
int main(int argc, char * argv[])
{
  ...
  // std::cout << "fk_anim size: " << fk_anim.size() << "\n";
  // for (int i11 = 0; i11 < fk_anim.size(); i11++) {
  //   std::cout << "fk_anim["<< i11 <<"] size: " << fk_anim[i11].size() << "\n";
  //   for (int i22 = 0; i22 < fk_anim[i11].size(); i22++) {
  //     std::cout << fk_anim[i11][i22].first << "   " << "(" << 
  //     fk_anim[i11][i22].second[0] << " " << fk_anim[i11][i22].second[1] << " " << fk_anim[i11][i22].second[2] << "), ";
  //     } std::cout<< "\n";
  // }
  ...
  const auto update = [&]()
  {
    ...
    const Eigen::RowVector3d pink(0.99608,0.76078,0.76078);
    {
      ...
      //std::cout << "main: tt_call: s\n";
      Eigen::VectorXd xb = transformed_tips(skeleton,b);
      //std::cout << "main: tt_call: f\n";
      P <<
    ...
    //std::cout << "main: fk_call: s\n";
    forward_kinematics(skeleton,T);
    //std::cout << "main: fk_call: f\n";
    // Apply bone transformations to deform shape
    //std::cout << "main: lbs_call: s\n";
    linear_blend_skinning(V,skeleton,T,W,U);
    //std::cout << "main: lbs_call: f\n";
  ...
  v.callback_pre_draw = [&](igl::opengl::glfw::Viewer &)->bool
  {
    ...
    // Forward Kinematice animation
    {
      ...
      {
        //std::cout << "main: cri_call: s\n";
        skeleton[b].xzx = catmull_rom_interpolation( fk_anim[b],anim_t);
        //std::cout << "main: cri_call: f\n";
      }
    }

    //std::cout << "main: update_call: s\n";
    update();
    //std::cout << "main: update_call: f\n";
    return false;
  };
